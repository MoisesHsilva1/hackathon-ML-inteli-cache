...
## 11. Implementação no SageMaker — Passo a Passo
...
3. **Políticas gerenciadas a anexar:**
   ...
   - `AmazonS3FullAccess` (ou a política customizada do arquivo `iam/sagemaker_policy.json`)
...
```bash
aws s3 cp src/recommender_ml.py s3://meu-bucket-polaris/code/recommender_ml.py
```
...
Faça upload do arquivo `lambdas/trigger.py` como código da função.
...
Faça upload do arquivo `lambdas/warmup.py`.
...
Para rodar o modelo diariamente mesmo sem novo CSV:
...
|---------|------------------|-------|
| S3 | ~$0.50 | < 1GB armazenado |
| Lambda (trigger) | ~$0.00 | Pouquíssimas invocações |
| Lambda (warmup) | ~$0.10 | 500 chamadas × 300ms |
| SageMaker Processing | ~$1.50 | 1 execução/dia × $0.23/hora × ~7min |
| CloudWatch Logs | ~$0.50 | Retenção padrão |
| **Total** | **~$2.60/mês** | Para 1 execução diária |
...
*Documentação gerada em 2026-05 · Cache Warmup Recommender v2.0 (Hybrid SVD)*
